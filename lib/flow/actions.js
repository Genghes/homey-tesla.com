...
function onAutonditioningTemperature (args) {
  ...
  return args.device.getApi().controlAutoConditioning(args.device.getStoreValue('vehicleId'), true)
  .then(response => args.device.getApi().setAutoConditioningTemperatures(args.device.getStoreValue('vehicleId'), args.temp, args.temp))
  .then(response => Promise.resolve(true))
  .catch(reason => Promise.reject(reason))
}
...
function onSettingsBool (args) {
  Util.debugLog('Flow action change setting bool', args.device.getData().id, [args.setting, args.value])
  let newSetting = {}
  newSetting[args.setting] = (args.value === 'ON')
  return args.device.setSettings(newSetting)
  .then(args.device.trackControllerBuffer())
  .then(response => Promise.resolve(true))
  .catch(reason => Promise.reject(reason))
}
