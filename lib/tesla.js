...
Tesla.prototype.login = function () {
  ...
  return login(self.user, self.password).then(function (grant) {
    ...
    console.log('error in Tesla.prototype.login', reason)
    if (reason === 'no_username' || reason === 'no_password') {
...
Tesla.prototype.validateGrant = function () {
  ...
  if (grantExpired(self.grant)) {
    console.log('validateGrant -> grantExpired')
    return tokenRefresh(self.grant.refresh_token).then((newGrant) => {
      console.log('validateGrant -> new token reveived')
...
Tesla.prototype.getVehicleState = async function (vehicleId) {
  var self = this
  try {
    await self.validateGrant()
    let vehicleState = await getVehicleCommand(self.grant.access_token, vehicleId, 'data_request/vehicle_state')
    return Promise.resolve(vehicleState)
  } catch (error) {
